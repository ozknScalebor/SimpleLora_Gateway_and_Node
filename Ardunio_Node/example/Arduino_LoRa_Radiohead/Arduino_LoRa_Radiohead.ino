...
RH_RF95::ModemConfig my_Bw125Cr45Sf4096 = {0x72, 0xc4, 0x0C};
...
uint8_t checkDataSize(char *ptr, uint8_t len)
{
  uint8_t sz = 0;
...
void loop() {
  ...
  dtostrf(SHT2x.GetHumidity(), 4, 2, humidity);
  dtostrf(SHT2x.GetTemperature(), 4, 2, temperature);
  
  sprintf(data, "Hum.(RH):%s,Temp.(C):%s", humidity, temperature);

  packetSize = checkDataSize(data, PACKET_SIZE);
   
  memcpy(message+app_key_offset,data,packetSize);
  ...
  sprintf(bufferStr, "Sending: ""%s""\r\n", data);

  Serial.write(bufferStr);
  
  manager.sendto(message,packetSize+app_key_offset,1);
